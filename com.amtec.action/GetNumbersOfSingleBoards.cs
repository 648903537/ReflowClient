...
namespace com.amtec.action
{
    public class GetNumbersOfSingleBoards
    {
        ...
        public List<MdataGetPartData> GetNumbersOfSingleBoardsResultCall(String partNumber)
        {
            ...
            if (result != 0)
            {
                string errorString = UtilityFunction.GetZHSErrorString(result, init, sessionContext);
                view.errorHandler(2, init.lang.ERROR_API_CALL_ERROR + " mdataGetPartData " + result + "(" + errorString + ")", "");
                return null;
            }
