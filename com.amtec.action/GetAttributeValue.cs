...
namespace com.amtec.action
{
    public class GetAttributeValue
    {
        ...
        public string GetAttributeValuesForStation()
        {
            ...
            {
                view.errorHandler(2, init.lang.ERROR_API_CALL_ERROR + " attribGetAttributeValues " + error + "(" + errorString + ")", "");
            }
            return returnValue;
        }
        ...
        public string GetAttributeValues(string serialNumber)
        {
            ...
            {
                string errorString = UtilityFunction.GetZHSErrorString(error, init, sessionContext);
                view.errorHandler(3, init.lang.ERROR_API_CALL_ERROR + " attribGetAttributeValues " + error + "(" + errorString + ")", "");
            }
            return returnValue;
        }
        ...
        public string GetObjectValeForSN(string stationNumber, string attriCode, string attriValue)//attriCode =*
        {
            ...
            {
                string errorString = UtilityFunction.GetZHSErrorString(error, init, sessionContext);
                view.errorHandler(2, init.lang.ERROR_API_CALL_ERROR + " attribGetObjectsForAttributeValues " + error + "(" + errorString + ")", "");
            }
            return serialNumber;
        }
        ...
        public Dictionary<string, string> GetAllAttributeValuesForWO(string workorder)
        {
            ...
            string errorString = UtilityFunction.GetZHSErrorString(error, init, sessionContext);
            LogHelper.Info("end api attribGetAttributeValues (result code = " + error + ", error message =" + errorString + ")");
            if (error == 0)
            {
                int loop = attributeResultKeys.Length;
                int count = attributeResultValues.Length;
